...
void AgentDoingTaskInfo::dump(string& result) const {
  ...
  CwxCommon::snprintf(buf, 2047, "msg_task_id=%u\ntask_id=%s\nsubtask_id=%s\ncmd_id=%s\nsvr_pool=%s\ntask_cmd=%s\ntask_result_file=%s\naction=%s",
    msg_task_id_, task_id_.c_str(), subtask_id_.c_str(), cmd_id_.c_str(), svr_pool_.c_str(), task_cmd_.c_str(), task_result_file_.c_str(), action_.c_str());
  result = buf;
  delete buf;
}
bool AgentDoingTaskInfo::load(string const& result) {
  ...
  if (!CwxCommon::findKey(kvs, "action", kv)) return false;
  action_ = CwxCommon::trim(kv.second);
  return true;
}
