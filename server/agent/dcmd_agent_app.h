...
namespace dcmd {
const char* const kDcmdAgentVersion = "1.0.8";
const char* const kDcmdAgentModifyDate = "2017-12-08 16:42:00";
// agent的app对象
class DcmdAgentApp : public CwxAppFramework{
 ...
 private:
  ...
  // 获取正在执行的subtask的文件
  inline string& GetDoingTaskFile(string& file, string const& subtask_id, string const& action) {
    GetDoingTaskPath(file);
    file += string("/") + subtask_id + "." + action;
    return file;
  }
  // 获取任务的执行的stdout、stderr结果的输出文件
  inline string& GetTaskOutputFile(string& file, string const& subtask_id, string const& action) {
    GetTaskOutputPath(file);
    file += string("/sub_task_") + subtask_id + "." + action + ".out";
    return file;
  }
  // 获取Task执行的结果文件
  inline string& GetTaskResultFile(string const& subtask_id, string const& action, string& file) {
    GetTaskOutputPath(file);
    file += string("/result_") + subtask_id + "." + action + string(".result");
    return file;
  }
  // 获取任务执行的script文件
  inline string& GetTaskRunScriptFile(string const& subtask_id, string const& action, string& file)
  {
    GetTaskScriptPath(file);
    file += string("/dcmd_task_") + subtask_id + "." + action +  ".script";
    return file;        
  }
  // 获取任务执行的环境变量脚本文件
  inline string& GetTaskRunScriptEnvFile(string const& subtask_id, string const& action, string& file) {
    GetTaskRunScriptFile(subtask_id, action, file);
    file += ".env.sh";
    return file;
  }
  // 获取任务执行的脚本shell文件
  inline string& GetTaskRunScriptShellFile(string const& subtask_id, string const& action, string& file)
  {
    GetTaskRunScriptFile(subtask_id, action, file);
    file += ".sh";
    return file;        
  ...
  // 获取服务脚本的shell文件名
  inline string& GetSvrScriptFile(string const& app_name, string const& svr_name, string const& action, string& file) {
    GetTaskScriptPath(file);
    file += string("/dcmd_svr_") + app_name + "_" + svr_name  + "." + action + ".script";
    return file;
  }
  // 获取操作的脚本目录
  ...
  // 处理控制指令
  void ExecCancelSubTask(AgentSvrPoolObj* svr_pool_obj, string const& subtask_id, string const& action);
  // 输出环境变量信息
  bool PrepareEnv(AgentTaskCmd* cmd, FILE* fd, string const& filename, string& err_msg);
  // 准备subtask命令运行的环境
  ...
  // 获取app任务执行的输出文件内容
  void LoadSubTaskResult(string const& subtask_id, string const& action, string& out_process, bool& is_success, string& err_msg, bool& is_finish,
  ...
  bool loadDoingTasks();
  //获取正在做的task的信息
  bool loadDoingTask(string const& subtask_file, AgentDoingTaskInfo& task_info);
  //删除正在做的task的信息
  bool removeDoingTask(string const& subtask_file);
  //保存正在做的task的信息
  bool saveDoingTask(AgentDoingTaskInfo const& task_info);
  // 根据Taskcmd，获取服务对应的pool
  AgentSvrPoolObj* GetSvrPool(AgentTaskCmd const* cmd);
  // 获取服务池的名字
  void GetSvrPoolName(AgentTaskCmd const* cmd, string& poolname) {
    poolname = cmd->cmd_.app_name() + "_" + cmd->cmd_.svr_name() + "_" + cmd->cmd_.svr_pool();
  }
