...
static int get_host_sn(string& sn) {
  char cmd[256];
  char filename[64];
  snprintf(filename, 64, "/tmp/agent_%d.sn", getpid());
  snprintf(cmd, 256, "dmidecode  -s system-serial-number >%s", filename);
  if (0 != system(cmd)) return -1;
  if (!CwxFile::readTxtFile(string(filename), sn)) return -1;
  CwxCommon::trim(sn, "\r\n");
  return 0;
}
...
int DcmdAgentApp::initRunEnv(){
  ...
  { // 获取设备序列号
    if (0 != get_host_sn(sn_)) {
      CWX_ERROR(("Failed to get host's sn, errno=%d", errno));
      return -1;
    }
    CWX_INFO(("Host's sn:%s", sn_.c_str()));
  }
  // 加载已经完成的任务
  loadDoingTasks();
...
int DcmdAgentApp::onConnCreated(CwxAppHandler4Msg& conn, bool& , bool& ) {
  ...
  report.set_sn(sn_);
  if (config_.conf().report_ip_.length())
    report.set_report_ip(config_.conf().report_ip_);
...
// 根据Taskcmd，获取服务对应的pool
AgentSvrPoolObj* DcmdAgentApp::GetSvrPool(AgentTaskCmd const* cmd)
{
  map<string, AgentSvrPoolObj*>::iterator svr_pool_iter;
  string poolname;
  GetSvrPoolName(cmd, poolname);
  svr_pool_iter = svr_pool_map_.find(poolname);
  if (svr_pool_iter != svr_pool_map_.end()) return svr_pool_iter->second;
  return NULL;
}
  ...
  if (recieved_subtasks_.begin() != recieved_subtasks_.end()){
    ...
    while(iter != recieved_subtasks_.end()){
      ...
      if ((*iter)->cmd_.cmd_type() != dcmd_api::CMD_DO_SUBTASK) {
        ...
        if (!svr_pool_obj){ // 创建新的池子
          svr_pool_obj = new AgentSvrPoolObj();
          GetSvrPoolName(*iter, svr_pool_obj->unique_name_);
          ...
          svr_pool_map_[svr_pool_obj->unique_name_] = svr_pool_obj;
        }
        // cancel存在的同一个subtask的命令
        if (((*iter)->cmd_.action() == "install") || ((*iter)->cmd_.action() == "uninstall")) {
          ExecCtrlTaskCmdForCancelAll(svr_pool_obj, *iter);
        } else {
          CWX_ASSERT((*iter)->cmd_.cmd_type() == dcmd_api::CMD_DO_SUBTASK);
          // 队列中只会有一个
          ExecCancelSubTask(svr_pool_obj, (*iter)->cmd_.subtask_id(), (*iter)->cmd_.action());
        }
...
void DcmdAgentApp::CheckSvrTask(AgentSvrPoolObj* svr_pool_obj) {
  ...
  if (!svr_pool_obj->processor_ && (svr_pool_obj->cmds_.begin() != svr_pool_obj->cmds_.end())) {
    ...
    GetTaskResultFile(cmd->cmd_.subtask_id(), cmd->cmd_.action(), task_info.task_result_file_);
    ...
    task_info.action_ = cmd->cmd_.action();
...
void DcmdAgentApp::CheckRuningSubTask(AgentSvrPoolObj* svr_pool_obj, bool is_cancel){
  ...
  if (!is_kill){
    LoadSubTaskResult(svr_pool_obj->running_cmd_->cmd_.subtask_id(),
      svr_pool_obj->running_cmd_->cmd_.action(),
      out_process,
      is_success,
      err_msg,
      is_finish,
  ...
  GetTaskRunScriptFile(svr_pool_obj->running_cmd_->cmd_.subtask_id(),
    svr_pool_obj->running_cmd_->cmd_.action(), script_file);
  GetTaskRunScriptShellFile(svr_pool_obj->running_cmd_->cmd_.subtask_id(),
    svr_pool_obj->running_cmd_->cmd_.action(), script_sh_file);
  CwxFile::rmFile(script_file.c_str());
  ...
  AgentTaskCmd* cmd)
{
    ...
    (svr_pool_obj->running_cmd_->cmd_.subtask_id() == cmd->cmd_.subtask_id()))
  {
    ...
    list<AgentTaskCmd*>::iterator iter_cancel;
    iter_cancel = svr_pool_obj->cmds_.begin();
    while(iter_cancel != svr_pool_obj->cmds_.end()){
      ...
      if ((*iter_cancel)->cmd_.subtask_id() ==  cmd->cmd_.subtask_id()) {
        ...
        result = new AgentTaskResult();
        FillTaskResult(*(*iter_cancel), *result, "", false,
          "Subtask is canceled.");
        wait_send_result_map_[result->cmd_id_] = result;
...
// 处理控制指令
void DcmdAgentApp::ExecCancelSubTask(AgentSvrPoolObj* svr_pool_obj, string const& subtask_id, string const& action) {
  AgentTaskResult* result = NULL;
  CWX_INFO(("Exec cancel command for all, svr=%s svr_pool=%s  subtask_id=%s  action=%s",
    svr_pool_obj->svr_name_.c_str(),
    svr_pool_obj->svr_pool_name_.c_str(),
    subtask_id.c_str(),
    action.c_str()));
  ...
  if (svr_pool_obj->running_cmd_ && 
    ...
    (svr_pool_obj->running_cmd_->cmd_.action() == action))
  {
    CWX_INFO(("Cancel running subtask, svr=%s, svr_pool=%s, sub_task=%s, action=%s",
      svr_pool_obj->svr_name_.c_str(),
      svr_pool_obj->svr_pool_name_.c_str(),
      subtask_id.c_str(),
      action.c_str()));
    ...
    while(iter_cancel != svr_pool_obj->cmds_.end()){
      if (((*iter_cancel)->cmd_.subtask_id() ==  subtask_id) && 
          ((*iter_cancel)->cmd_.action() == action)){
        CWX_INFO(("Cancel subtask, svr=%s, svr_pool=%s, sub_task=%s, action=%s",
          svr_pool_obj->svr_name_.c_str(),
          svr_pool_obj->svr_pool_name_.c_str(),
          subtask_id.c_str(),
          action.c_str()));
...
// 输出环境变量信息
bool DcmdAgentApp::PrepareEnv(AgentTaskCmd* cmd, FILE* fd, string const& filename, string& err_msg) {
  string env_file;
  string svr_script_file;
  string result_file;
  bool is_success = false;
  GetTaskRunScriptEnvFile(cmd->cmd_.subtask_id(), cmd->cmd_.action(), env_file);
  GetSvrScriptFile(cmd->cmd_.app_name(), cmd->cmd_.svr_name(), cmd->cmd_.action(), svr_script_file);
  GetTaskResultFile(cmd->cmd_.subtask_id(), cmd->cmd_.action(), result_file);
  do {
    ...
    if (!OutputShellEnv(fd, "DCMD_ACTION", cmd->cmd_.action(), err_2k_, filename.c_str())) break;
    if (!OutputShellEnv(fd, "DCMD_SVR_ENV_FILE", env_file, err_2k_, filename.c_str())) break;
    if (!OutputShellEnv(fd, "DCMD_SVR_POOL", cmd->cmd_.svr_pool(), err_2k_, filename.c_str())) break;
    if (!OutputShellEnv(fd, "DCMD_SVR_NAME", cmd->cmd_.svr_name(), err_2k_, filename.c_str())) break;
    if (!OutputShellEnv(fd, "DCMD_APP_NAME", cmd->cmd_.app_name(), err_2k_, filename.c_str())) break;
    if (!OutputShellEnv(fd, "DCMD_SVR_SCRIPT_FILE", svr_script_file, err_2k_, filename.c_str())) break;
    if (!OutputShellEnv(fd, "DCMD_SVR_VERSION", cmd->cmd_.svr_ver(), err_2k_, filename.c_str())) break;
    if (!OutputShellEnv(fd, "DCMD_SVR_VERSION_MD5", cmd->cmd_.svr_ver_md5(), err_2k_, filename.c_str())) break;
    if (!OutputShellEnv(fd, "DCMD_SVR_VERSION_PASSWD", cmd->cmd_.svr_ver_passwd(), err_2k_, filename.c_str())) break;
    if (!OutputShellEnv(fd, "DCMD_SVR_REPO", cmd->cmd_.svr_repo(), err_2k_, filename.c_str())) break;
    if (!OutputShellEnv(fd, "DCMD_SVR_USER", cmd->cmd_.svr_user(), err_2k_, filename.c_str())) break;
    if (!OutputShellEnv(fd, "DCMD_SVR_IP", cmd->cmd_.ip(), err_2k_, filename.c_str())) break;
    if (!OutputShellEnv(fd, "DCMD_SVR_PATH", cmd->cmd_.svr_path(), err_2k_, filename.c_str())) break;
    ...
    if (!OutputShellEnv(fd, "DCMD_SVR_CONF_V", cmd->cmd_.svr_env_ver(), err_2k_, filename.c_str())) break;
    if (!OutputShellEnv(fd, "DCMD_SVR_CONF_MD5", cmd->cmd_.svr_env_md5(), err_2k_, filename.c_str())) break;
    if (!OutputShellEnv(fd, "DCMD_SVR_CONF_PASSWD", cmd->cmd_.svr_env_passwd(), err_2k_, filename.c_str())) break;
    if (!OutputShellEnv(fd, "DCMD_SVR_ENV_V", cmd->cmd_.svr_env_ver(), err_2k_, filename.c_str())) break;
    if (!OutputShellEnv(fd, "DCMD_SVR_ENV_MD5", cmd->cmd_.svr_env_md5(), err_2k_, filename.c_str())) break;
    if (!OutputShellEnv(fd, "DCMD_SVR_ENV_PASSWD", cmd->cmd_.svr_env_passwd(), err_2k_, filename.c_str())) break;
    ...
    for (i = 0; i<arg_num; i++) {
      ...
      if (!OutputShellEnv(fd, key.c_str(), value, err_2k_, filename.c_str())) break;
    }
    if (i < arg_num) break;
    //输出服务池子的参数
    ...
    if (i < arg_num) break;
    // 输出env信息
    arg_num = cmd->cmd_.env_arg_size();
    for (i = 0; i<arg_num; i++) {
      key = string("DCMD_SVR_CLOUD_ENV_") + cmd->cmd_.env_arg(i).key();
      dcmd_remove_spam(key);
      value = cmd->cmd_.env_arg(i).value();
      if (!OutputShellEnv(fd, key.c_str(), value, err_2k_, filename.c_str())) break;
    }
    if (i < arg_num) break;
    // 输出sys信息
    arg_num = cmd->cmd_.sys_arg_size();
    for (i = 0; i<arg_num; i++) {
      key = string("DCMD_SVR_SYS_") + cmd->cmd_.sys_arg(i).key();
      dcmd_remove_spam(key);
      value = cmd->cmd_.sys_arg(i).value();
      if (!OutputShellEnv(fd, key.c_str(), value, err_2k_, filename.c_str())) break;
    }
    if (i < arg_num) break;
    is_success = true;
  }while (0);
  if (!is_success) err_msg = err_2k_;
  return is_success;
}
...
bool DcmdAgentApp::PrepareSubtaskRunEnv(AgentTaskCmd* cmd, string& err_msg) {
  ...
  GetTaskOutputFile(output_file, cmd->cmd_.subtask_id(), cmd->cmd_.action());
  if (CwxFile::isFile(output_file.c_str())) CwxFile::rmFile(output_file.c_str());
  GetTaskRunScriptFile(cmd->cmd_.subtask_id(), cmd->cmd_.action(), script_file);
  ...
  GetTaskRunScriptShellFile(cmd->cmd_.subtask_id(), cmd->cmd_.action(), script_sh_file);
  if (CwxFile::isFile(script_sh_file.c_str())) CwxFile::rmFile(script_sh_file.c_str());
  GetSvrScriptFile(cmd->cmd_.app_name(), cmd->cmd_.svr_name(), cmd->cmd_.action(), svr_script_file);
  // 保存svr script file
  {
    if (!cmd->cmd_.svr_script().length()) {
      CwxCommon::snprintf(err_2k_, 2047, "subtask's svr_script is empty.");
      err_msg = err_2k_;
      return false;
    }
    fd = fopen(svr_script_file.c_str(), "w+");
    if (!fd) {
      CwxCommon::snprintf(err_2k_, 2047, "Failure to open subtask's svr-script file:%s, errno=%d",
        svr_script_file.c_str(), errno);
      err_msg = err_2k_;
      return false;
    }
    if (fwrite(cmd->cmd_.svr_script().c_str(), 1, cmd->cmd_.svr_script().length(), fd)
      != cmd->cmd_.svr_script().length())
    {
      fclose(fd);
      CwxCommon::snprintf(err_2k_, 2047, "Failure to write subtask's svr-script file:%s, errno=%d",
        svr_script_file.c_str(), errno);
      err_msg = err_2k_;
      return false;
    }
    fclose(fd);
    fd = NULL;
    if (0 != chmod(svr_script_file.c_str(), S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IWGRP | S_IXGRP | S_IROTH | S_IWOTH | S_IXOTH)) {
      CwxCommon::snprintf(err_2k_, 2047, "Failure to set 'rwx-rwx-rwx' permissions for svr script file:%s, errno=%d.",
        svr_script_file.c_str(), errno);
      err_msg = err_2k_;
      return false;
    }
  ...
  do {
    ...
    if (!fd) {
      CwxCommon::snprintf(err_2k_, 2047, "Failure to open subtask's env file:%s, errno=%d",
        env_file.c_str(), errno);
      err_msg = err_2k_;
      break;
    }
    if (!PrepareEnv(cmd, fd, script_sh_file, err_msg)) break;
    ::fclose(fd);
    // 写运行shell脚本文件
  ...
  if (is_success){// 修改shell文件的权限
    ...
    if (0 != chmod(env_file.c_str(), S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IWGRP | S_IXGRP | S_IROTH | S_IWOTH | S_IXOTH)) {
      CwxCommon::snprintf(err_2k_, 2047, "Failure to set 'rwx-rwx-rwx' permissions for task script file:%s, errno=%d.",
        env_file.c_str(), errno);
      err_msg = err_2k_;
      return false;
...
void DcmdAgentApp::FillTaskResult(AgentTaskCmd const& cmd, AgentTaskResult& result,
  ...
  GetTaskResultFile(cmd.cmd_.subtask_id(), cmd.cmd_.action(), result.result_file_);
  ...
  result.result_.set_action(cmd.cmd_.action());
...
bool DcmdAgentApp::ExecSubTaskCmd(AgentTaskCmd* cmd, string& err_msg,
  DcmdProcess*& process) {
  string script_file;
  GetTaskRunScriptShellFile(cmd->cmd_.subtask_id(), cmd->cmd_.action(), script_file);
  process = new DcmdProcess(script_file);
  if (!process->Run(NULL, NULL, NULL, &err_msg)) {
    delete process;
    process = NULL;
    return false;
  }
...
int DcmdAgentApp::SubTaskResultReply(CwxMsgBlock*& msg, AgentCenter* center){
  ...
  if (iter != wait_reply_result_map_.end()){
    CWX_INFO(("Receive result confirm for cmd:%s from center:%s",
      ...
      center->host_name_.c_str()));
    //删除doing-task文件
    string doing_task_file;
    GetDoingTaskFile(doing_task_file, iter->second->result_.subtask_id(), iter->second->result_.action());
    removeDoingTask(doing_task_file);
    //删除输出文件
...
int DcmdAgentApp::FetchTaskOutputResultRecieved(CwxMsgBlock*& msg, AgentCenter* center) {
  ...
  string file_name;
  // 获取任务指令输出文件
  GetTaskOutputFile(file_name, recv.subtask_id(), recv.action());
  CWX_DEBUG(("Receive subtask output command from center:%s, subtask-id=%s, action=%s, offset=%u, file=%s ",
    center->host_name_.c_str(),
  ...
  bool is_process_only)
{
  ...
  err_msg.erase();
  // 获取文件
  GetTaskResultFile(subtask_id, action, out_filename);
...
bool DcmdAgentApp::loadDoingTasks() {
  ...
  while(iter != files.end()) {
    subtask_file = subtask_path + "/" + *iter;
    if (!loadDoingTask(subtask_file, task_info)) {
      removeDoingTask(subtask_file);
    } else {
      // load 进程的process
      if (CwxFile::isFile(task_info.task_result_file_.c_str())) {
        LoadSubTaskResult(task_info.subtask_id_, task_info.action_, out_process, is_success, err_msg, is_finish, false);
        if (is_finish) {
          //将任务结果放到等待发送map
          ...
          task_result->result_.set_action(task_info.action_);
...
//获取正在做的task的信息
bool DcmdAgentApp::loadDoingTask(string const& subtask_file, AgentDoingTaskInfo& task_info) {
  if (!CwxFile::isFile(subtask_file.c_str())) {
    CWX_ERROR(("Doing-task doesn't exist, subtask_file:%s", subtask_file.c_str()));
    return false;
  }
  string file_data;
  if (!CwxFile::readTxtFile(subtask_file.c_str(), file_data)) {
    CWX_ERROR(("Failure to read doing-task, subtask_file:%s", subtask_file.c_str()));
    return false;
  }
  return task_info.load(file_data);
}
//删除正在做的task的信息
bool DcmdAgentApp::removeDoingTask(string const& subtask_file) {
...
bool DcmdAgentApp::saveDoingTask(AgentDoingTaskInfo const& task_info) {
  ...
  GetDoingTaskFile(subtask_file, task_info.subtask_id_, task_info.action_);
