...
class DcmdCenterConfCmn{
 public:
   enum {
     // agent采用IP认证
     AGENT_AUTH_TYPE_IP = 1,
     // agent采用sn认证
     AGENT_AUTH_TYPE_SN = 2,
     // agent采用混合认证,先识别IP，若IP无法识别则通过SN
     AGENT_AUTH_TYPE_IP_SN = 3,
     // agent采用混合认证,先识别SN，若SN无法识别则通过IP
     AGENT_AUTH_TYPE_SN_IP = 4
   };
  ...
  DcmdCenterConfCmn(){
    ...
    auth_type_ = AGENT_AUTH_TYPE_IP; //缺省是IP认证
  }
 public:
  // 工作目录
  string              work_home_;
  ...
  // 运行连接center的ip地址，可以为a,b,c类段
  set<string>          allow_ui_ips_;
  // 认证类型字符串
  string               str_auth_type_;
  // 认证类型
  uint8_t              auth_type_;
};
// 控制中心的mysql连接信息
...
class DcmdCenterConf {
 ...
 public:
  ...
  // 获取服务命令的文件名
  inline static string& svr_file(string const& path, string const& app_name, string const& svr_name, string const& pool_group, string& cmd_file) {
    if (!pool_group.length())
      cmd_file = path + app_name + "/" + string(kSvrFilePrex) + app_name + "_" + svr_name + kSvrFileSuffix;
    else
      cmd_file = path + app_name + "/" + string(kSvrFilePrex) + app_name + "_" + svr_name + "_" + pool_group + kSvrFileSuffix;
    return cmd_file;
  }
  // 获取脚本文件
