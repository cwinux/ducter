...
int DcmdCenterConf::Init(string const& conf_file) {
  ...
  // load common:svr_script_path
  if (!cnf.getAttr("common", "svr_script_path", value) || !value.length()) {
    snprintf(err_msg_, 2047, "Must set [common:svr_script_path].");
    return -1;
  }
  common_.svr_script_path_ = value;
  if ('/' != value[value.length() - 1]) common_.svr_script_path_ += "/";
  ...
  // load common::agent_auth_type
  if (cnf.getAttr("common", "agent_auth_type", value) && value.length()) {
    CwxCommon::trim(value);
    common_.str_auth_type_ = value;
    if (value.length()) {
      if (value == "ip")
        common_.auth_type_ = DcmdCenterConfCmn::AGENT_AUTH_TYPE_IP;
      else if (value == "sn")
        common_.auth_type_ = DcmdCenterConfCmn::AGENT_AUTH_TYPE_SN;
      else if (value == "ip;sn")
        common_.auth_type_ = DcmdCenterConfCmn::AGENT_AUTH_TYPE_IP_SN;
      else if (value == "sn:ip")
        common_.auth_type_ = DcmdCenterConfCmn::AGENT_AUTH_TYPE_SN_IP;
      else {
        snprintf(err_msg_, 2047, "[common:agent_auth_type] is wrong, must be [ip or sn or ip;sn or sn:ip]");
        return -1;
      }
    }
  }
...
void DcmdCenterConf::OutputConfig() const {
  ...
  CWX_INFO(("svr_script_path=%s", common_.svr_script_path_.c_str()));
  ...
  CWX_INFO(("agent_auth_type=%s", common_.str_auth_type_.length() ? common_.str_auth_type_.c_str() : ""));
  CWX_INFO(("*****************mysql conf*******************"));
