...
void DcmdCenterH4Admin::QueryScriptContent(CwxMsgBlock*& msg, DcmdTss* tss) {
  ...
  // 获取文件内容
  string script_file;
  if (dcmd_api::SCRIPT_TYPE_SVR == script_query.script_type()) {
    list<string> app_svr;
    CwxCommon::split(script_query.script_file(), app_svr, ':');
    if (app_svr.size() != 2) {
      CwxCommon::snprintf(tss->m_szBuf2K, 2047,
        "Filename[%s] must be [appname:svr_name]",
    ...
    string app_name = *app_svr.begin(); app_svr.pop_front();
    string svr_name = *app_svr.begin();
    DcmdCenterConf::svr_file(app_->config().common().svr_script_path_, app_name, svr_name, "", script_file);
  }else {
