...
class DcmdCenterTaskMgr{
 ...
 private:
  ...
  // 读取服务脚本的内容
  bool ReadSvrScriptContent(DcmdTss* tss, string const& app_name, string const& svr_name,  string const& pool_group, string& content);
  ...
  bool FreezeConflictDeployTask(DcmdTss* tss, DcmdCenterTask* task);
 private:
   // 根据svr id获取服务的脚本。返回值0：成功; -1：失败
   int FetchSvrScriptFromDb(DcmdTss* tss, uint32_t svr_id, map<string, pair<string, string> >& scripts);
   ...
   inline void FillCtrlCmd(dcmd_api::AgentTaskCmd& cmd, uint64_t cmd_id,
     dcmd_api::CmdType cmd_type, string const& agent_ip, string const& app_name,
