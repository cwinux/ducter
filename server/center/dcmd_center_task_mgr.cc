...
bool DcmdCenterTaskMgr::LoadNewTask(DcmdTss* tss, bool is_first) {
  ...
  CwxCommon::snprintf(tss->sql_, DcmdTss::kMaxSqlBufSize, "select task_id, task_name, task_cmd, depend_task_id,"\
    ...
    "err_msg, tag_md5, tag_passwd, is_deploy from dcmd_task where task_id > %d order by task_id asc", next_task_id_);
  ...
  while(1 == mysql_->next()){
    ...
    task->tag_md5_ = mysql_->fetch(23, is_null);
    task->tag_passwd_ = mysql_->fetch(24, is_null);
    task->is_deploy_ = strtoul(mysql_->fetch(25, is_null), NULL, 10) ? true : false;
...
bool DcmdCenterTaskMgr::LoadTaskSvrPool(DcmdTss* tss, DcmdCenterTask* task) {
  CwxCommon::snprintf(tss->sql_, DcmdTss::kMaxSqlBufSize,
    "select svr_pool, svr_pool_id, env_ver, repo, pool_group, run_user, undo_node, doing_node, "\
    "finish_node, fail_node, ignored_fail_node, ignored_doing_node, state, env_md5, env_passwd from dcmd_task_service_pool where task_id=%u",
    task->task_id_);
  if (!mysql_->query(tss->sql_)) {
  ...
  while(1 == mysql_->next()){
    ...
    svr_pool->pool_group_ = mysql_->fetch(4, is_null);
    svr_pool->run_user_ = mysql_->fetch(5, is_null);
    svr_pool->undo_subtask_num_ = strtoul(mysql_->fetch(6, is_null), NULL, 10);
    svr_pool->doing_subtask_num_ = strtoul(mysql_->fetch(7, is_null), NULL, 10);
    svr_pool->finished_subtask_num_ = strtoul(mysql_->fetch(8, is_null), NULL, 10);
    svr_pool->failed_subtask_num_ = strtoul(mysql_->fetch(9, is_null), NULL, 10);
    svr_pool->ignored_failed_subtask_num_ = strtoul(mysql_->fetch(10, is_null), NULL, 10);
    svr_pool->ignored_doing_subtask_num_ = strtoul(mysql_->fetch(11, is_null), NULL, 10);
    svr_pool->state_ = strtoul(mysql_->fetch(12, is_null), NULL, 10);
    svr_pool->svr_env_md5_ = mysql_->fetch(13, is_null);
    svr_pool->svr_env_passwd_ = mysql_->fetch(14, is_null);
    // 将service pool添加到任务中
...
bool DcmdCenterTaskMgr::AnalizeTask(DcmdTss* tss, DcmdCenterTask* task) {
  ...
  do {
    ...
    // load svr_script文件
    map < string, pair<string, string> > scripts;
    map < string, pair<string, string> >::iterator script_iter;
    if (-1 == FetchSvrScriptFromDb(tss, task->svr_id_, scripts))
      return false;
    map<string, DcmdCenterSvrPool*>::iterator iter = task->pools_.begin();
    while (iter != task->pools_.end()) {
      script_iter = scripts.find(iter->second->pool_group_);
      if (script_iter == scripts.end()) {
        if (iter->second->pool_group_.length()) script_iter = scripts.find("");
      }
      if (script_iter == scripts.end()) {
        if (!ReadSvrScriptContent(tss, task->app_name_, task->service_, iter->second->pool_group_, iter->second->svr_script_)) {
          task->is_valid_ = false;
          task->err_msg_ = string("Service pool[") + iter->second->svr_pool_ + string("]'s svr-script is missing, pool_group:") + iter->second->pool_group_;
          break;
        }
        dcmd_md5(iter->second->svr_script_.c_str(), iter->second->svr_script_.length(),
          iter->second->svr_script_md5_);
      }
      else {
        iter->second->svr_script_ = script_iter->second.second;
        iter->second->svr_script_md5_ = script_iter->second.first;
      }
      ++iter;
    }
    // load task_cmd文件
...
bool DcmdCenterTaskMgr::ReadSvrScriptContent(DcmdTss* tss, string const& app_name, string const& svr_name, string const& pool_group, string& content) {
  string script_file;
  script_file = DcmdCenterConf::svr_file(app_->config().common().svr_script_path_,
    app_name, svr_name, pool_group, script_file);
  if (!CwxFile::isFile(script_file.c_str())) {
    script_file = DcmdCenterConf::svr_file(app_->config().common().svr_script_path_,
      app_name, svr_name, "", script_file);
    if (!CwxFile::isFile(script_file.c_str())) {
      CwxCommon::snprintf(tss->m_szBuf2K, 2047, "svr-script doesn't exist, script file:%s",
        script_file.c_str());
      tss->err_msg_ = tss->m_szBuf2K;
      return false;
    }
  ...
  bool ret = CwxFile::readTxtFile(script_file, content);
  if (!ret) {
    CwxCommon::snprintf(tss->m_szBuf2K, 2047, "Failure to read svr script[%s], errno=%d",
      script_file.c_str(), errno);
    tss->err_msg_ = tss->m_szBuf2K;
    return false;
  }
  if (!content.length()) {
    CwxCommon::snprintf(tss->m_szBuf2K, 2047, "svr script is empty,script:%s",
      script_file.c_str());
    tss->err_msg_ = tss->m_szBuf2K;
    return false;
  }
...
// 根据svr id获取服务的脚本。返回值0：成功；-1：失败
int DcmdCenterTaskMgr::FetchSvrScriptFromDb(DcmdTss* tss, uint32_t svr_id, map<string, pair<string, string> >& scripts) {
  CwxCommon::snprintf(tss->sql_, DcmdTss::kMaxSqlBufSize,
    "select pool_group, script_md5, script from dcmd_service_script where svr_id =%d", svr_id);
  if (!mysql_->query(tss->sql_)) {
    CwxCommon::snprintf(tss->m_szBuf2K, 2047, "Failure to fetch service[%d]'s svr-script. err:%s; sql:%s",
      svr_id, mysql_->getErrMsg(), tss->sql_);
    tss->err_msg_ = tss->m_szBuf2K;
    mysql_->freeResult();
    return -1;
  }
  ...
  while (1 == mysql_->next()) {
    pool_group = mysql_->fetch(0, is_null);
    item.first = mysql_->fetch(1, is_null);
    item.second = mysql_->fetch(2, is_null);
    scripts[pool_group] = item;
  }
  mysql_->freeResult();
  return 0;
}
  ...
  DcmdCenterTask*& task )
{
  ...
  if (iter->second->subtask_->exec_cmd_ == iter->second) {
   ...
   if (iter->second->subtask_->task_->is_deploy_ && (result.action() == "install")) {
     string ver = iter->second->subtask_->task_->tag_;
     string ver_md5 = iter->second->subtask_->task_->tag_md5_;
     string conf_ver = iter->second->subtask_->svr_pool_->svr_env_ver_;
     string conf_ver_md5 = iter->second->subtask_->svr_pool_->svr_env_md5_;
     dcmd_escape_mysql_string(ver);
     dcmd_escape_mysql_string(ver_md5);
     dcmd_escape_mysql_string(conf_ver);
     dcmd_escape_mysql_string(conf_ver_md5);
     if (result.success()) {
       CwxCommon::snprintf(tss->sql_, DcmdTss::kMaxSqlBufSize,
         "update dcmd_service_pool_node set tag='%s', tag_md5='%s',env_ver='%s', env_md5='%s', tag_task_id=%d "\
         "where svr_pool_id=%d and ip='%s'",
         ver.c_str(), ver_md5.c_str(), conf_ver.c_str(), conf_ver_md5.c_str(),
         iter->second->subtask_->task_->task_id_, iter->second->subtask_->svr_pool_->svr_pool_id_,
         iter->second->subtask_->ip_.c_str());
     } else {
       CwxCommon::snprintf(tss->sql_, DcmdTss::kMaxSqlBufSize,
         "update dcmd_service_pool_node set tag='%s', tag_md5='',env_ver='%s', env_md5='', tag_task_id=%d "\
         "where svr_pool_id=%d and ip='%s'",
         ver.c_str(), conf_ver.c_str(),
         iter->second->subtask_->task_->task_id_, iter->second->subtask_->svr_pool_->svr_pool_id_,
         iter->second->subtask_->ip_.c_str());
     }
  ...
  uint32_t uid)
{
  ...
  // finish 冲突的任务
  if (task->is_deploy_ && !FreezeConflictDeployTask(tss, task)) {
    return dcmd_api::DCMD_STATE_FAILED;
  }
  ...
  while(iter != task->pools_.end()) {
    ...
    if (task->is_deploy_) {
      string ver = task->tag_;
      string ver_md5 = task->tag_md5_;
      dcmd_escape_mysql_string(ver);
      dcmd_escape_mysql_string(ver_md5);
      CwxCommon::snprintf(tss->sql_, DcmdTss::kMaxSqlBufSize,
        "update dcmd_service_pool set tag='%s', tag_md5='%s',tag_task_id=%d "\
        "where svr_pool_id=%d ",
        ver.c_str(), ver_md5.c_str(), task->task_id_, iter->second->svr_pool_id_);
      if (!ExecSql(tss, false)) {
        mysql_->disconnect();
        return dcmd_api::DCMD_STATE_FAILED;
      }
...
// freeze与当前发布任务冲突的其他发布任务
bool DcmdCenterTaskMgr::FreezeConflictDeployTask(DcmdTss* tss, DcmdCenterTask* task) {
  CWX_ASSERT(task->is_deploy_);
  bool is_commit = false;
  map<string, DcmdCenterSvrPool*>::iterator pool_iter;
  map<uint32_t, DcmdCenterTask*>::iterator iter = all_tasks_.begin();
  DcmdCenterTask* free_task = NULL;
  while (iter != all_tasks_.end()) {
    free_task = iter->second;
    if (free_task->is_deploy_ && !free_task->is_freezed_ && (free_task->task_id_ != task->task_id_) && (free_task->svr_id_ == task->svr_id_)) {
      pool_iter = task->pools_.begin();
      while (pool_iter != task->pools_.end()) {
        if (free_task->pools_.find(pool_iter->first) != free_task->pools_.end()) {
          CwxCommon::snprintf(tss->sql_, DcmdTss::kMaxSqlBufSize,
            "update dcmd_task set freeze=1 where task_id = %u", free_task->task_id_);
          if (!ExecSql(tss, false)) {
  ...
  uint32_t uid)
{
  ...
  FillCtrlCmd(cmd, cmd_id, dcmd_api::CMD_CANCEL_SUBTASK, subtask->ip_,
    subtask->task_->app_name_, subtask->service_, subtask->svr_pool_name_, subtask);
  ...
  char const* serivce_pool, char const* agent_ip, uint32_t uid)
{
  ...
  FillCtrlCmd(cmd, cmd_id, dcmd_api::CMD_CANCEL_SVR_SUBTASK, agent_ip,
    task->app_name_, task->service_, serivce_pool, NULL);
  ...
  uint32_t uid)
{
  ...
  while(iter != task->pools_.end()) {
    ...
    while(subtask_iter != iter->second->all_subtasks_.end()){
      ...
      subtask_iter->second->finish_time_ = time(NULL);
      subtask_iter->second->state_ = dcmd_api::SUBTASK_INIT;
      subtask_iter->second->check_start_time_ = 0;
      subtask_iter->second->is_ignored_ = false;
      subtask_iter->second->process_ = "";
      subtask_iter->second->err_msg_ = "";
      if (subtask_iter->second->exec_cmd_) RemoveCmd(subtask_iter->second->exec_cmd_);
  ...
  uint32_t uid)
{
  ...
  if (task->is_deploy_){
    ...
    map<uint32_t, DcmdCenterTask*>::iterator iter = all_tasks_.begin();
    while (iter != all_tasks_.end()) {
      if (iter->second->is_deploy_ && !iter->second->is_freezed_ && (iter->second->task_id_ != task->task_id_) && (iter->second->svr_id_ == task->svr_id_)) {
        pool_iter = task->pools_.begin();
        while (pool_iter != task->pools_.end()) {
    ...
    while (pool_iter != task->pools_.end()) {
      ...
      CwxCommon::snprintf(tss->sql_, DcmdTss::kMaxSqlBufSize,
        "update dcmd_service_pool set tag='%s', tag_md5='%s',tag_task_id=%d "\
        "where svr_pool_id=%d ",
        ver.c_str(), ver_md5.c_str(), task->task_id_, pool_iter->second->svr_pool_id_);
      if (!ExecSql(tss, false)) {
        CWX_ERROR((tss->err_msg_.c_str()));
        mysql_->disconnect();
        return dcmd_api::DCMD_STATE_FAILED;
  ...
  DcmdCenterSubtask const& subtask) 
{
  ...
  cmd.set_svr_ver_md5(subtask.task_->tag_md5_);
  cmd.set_svr_ver_passwd(subtask.task_->tag_passwd_);
  ...
  cmd.set_svr_env_md5(subtask.svr_pool_->svr_env_md5_);
  cmd.set_svr_env_passwd(subtask.svr_pool_->svr_env_passwd_);
  ...
  cmd.set_svr_script(subtask.svr_pool_->svr_script_);
  cmd.set_action("install");
  dcmd_api::KeyValue* kv = NULL;
  kv = cmd.add_sys_arg();
  kv->set_key("POOL_GROUP");
  kv->set_value(subtask.svr_pool_->pool_group_);
