...
<?php
$disabled = "";
if($task->freeze == 1 || $task->is_deploy) $disabled = "disabled";
?>
    <p>
      <!--<a href="<?= Url::to(['dcmd-task-service-pool/index', 'task_id'=>$task_id]) ?>">-->服务池子:<!--</a>-->  <?= Html::a('添加设备', Url::to(['dcmd-task-node/add', 'task_id'=>$task_id]), ['class' => 'btn btn-success', 'target'=>'_blank', $disabled=>true]) ?>
      &nbsp;&nbsp;
    ...
    <?= GridView::widget([
        ...
        'columns' => [
            ...
            array('attribute'=>'env_ver','label'=>'配置版本', 'enableSorting'=>false,),
            ...
            array('attribute'=>'state', 'label'=>'状态', 'enableSorting'=>false, 'filter'=>false, 'content'=>function($model, $key, $index, $col) { if($model['state'] == 0) return "未提交"; if($model['state'] == 1) return "正在执行"; if($model['state'] == 2) return "失败"; if ($model['state']==3) return "完成"; if($model['state']==4) return "完成但有失败"; }),
