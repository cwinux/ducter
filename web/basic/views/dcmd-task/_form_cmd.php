...
var getServicePool = function() {
  ...
  $('#dcmdtask-tag').load("?r=dcmd-task/get-service-version&svr_id="+svr_id); 
  return 0;
}
...
<script>
  function verchange(){
    ...
    var svr = $("#dcmdtask-svr_id option:selected");
    $.get(url,{version:options.val(),svr_id:svr.val()},function(data){
      data = $.parseJSON(data);
//      alert(data);
