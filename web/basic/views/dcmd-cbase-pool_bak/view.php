...
<div class="dcmd-service-pool-node-view" id="dcmd-service-pool-node">
    <?= GridView::widget([
        ...
        'columns' => [
            ...
            array('attribute'=>'ip', 'label'=>'服务器状态','enableSorting'=>false, 'filter'=>false,  'content'=>function($model, $key,$index, $column) { return $model->getAgentState($model['ip']);}),
            array('attribute'=>'ip','label'=>'Swap Usage', 'enableSorting'=>false, 'content' => function($model, $key, $index, $column) { return Html::a($model["ip"], Url::to(['dcmd-node/view-ip', 'ip'=>$model['ip']]));},),
            array('attribute'=>'ip', 'label'=>'RAM Usage', 'enableSorting'=>false, 'filter'=>false),
            array('attribute'=>'ip', 'label'=>'CPU Usage','enableSorting'=>false, 'filter'=>false,  'content'=>function($model, $key,$index, $column) { return $model->getAgentState($model['ip']);}),
            array('attribute'=>'ip', 'label'=>'Active Items','enableSorting'=>false, 'filter'=>false,  'content'=>function($model, $key,$index, $column) { return $model->getAgentState($model['ip']);}),
