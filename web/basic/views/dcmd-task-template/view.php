...
<div id="task-tmpate-view"  style="background:#f1f1f1;padding:10px;margin-top:10px">
    <?= DetailView::widget([
        ...
        'attributes' => [
            ...
            array('attribute'=>'update_env', 'label'=>'更新配置', 'value'=>$model->convert($model['update_env'])),
            array('attribute'=>'concurrent_rate', 'label'=>'并发数'),
            array('attribute'=>'timeout','label'=>'超时'),
            array('attribute'=>'process', 'label'=>'输出进度', 'value'=>$model->convert($model['process'])),
            array('attribute'=>'auto','label'=>'自动执行', 'value'=>$model->convert($model['auto'])),
    ...
    <?= GridView::widget([
       ...
       /// 'filterModel' => $searchModel,
        'columns' => [
            ...
            ['class' => 'yii\grid\ActionColumn', 'template'=>'{delete}', 'urlCreator'=>function($action, $model, $key, $index) {if ("delete" == $action) return Url::to(['dcmd-task-template-service-pool/delete', 'id'=>$model['id'], 'task_tmpt_id'=>$model['task_tmpt_id']]);}, "visible"=>(Yii::$app->user->getIdentity()->admin == 1) ? true : false],
        ],
    ]); ?>
    <p>
        <?= Html::a('添加', ['dcmd-task-template-service-pool/create', 'task_tmpt_id'=>$model['task_tmpt_id']], ['class' => 'btn btn-success', (Yii::$app->user->getIdentity()->admin == 1) ? "" : ""]) ?>
    </p>
