...
<div>
    <?= GridView::widget([
        ...
        'columns' => [
            ...
            ['class' => 'yii\grid\ActionColumn', 'urlCreator'=>function($action, $model, $key, $index) {if ("view" == $action) return Url::to(['dcmd-network/ips','id'=>$model['id']]);else if("update" == $action) return Url::to(['dcmd-network/subnet-update','id'=>$model['id']]);else return Url::to(['dcmd-network/subnet-delete','id'=>$model['id']]);}, "visible"=>(Yii::$app->user->getIdentity()->admin == 1) ? true : false],
        ],
    ]); ?>
    <p>
