...
<div class="dcmd-group-view"  id="dcmd-group" >
    ...
    <?= DetailView::widget([
        ...
        'attributes' => [
            ...
            array('attribute'=>'comp_id', 'label'=>'公司', 'value'=>$model->getCompany($model['comp_id'])),
...
<div class="dcmd-group-user-view" style="display:none" id="dcmd-group-user" >
    <?= GridView::widget([
        ...
        'columns' => [
            ...
            array('attribute'=>'is_leader', 'label'=>'是否Leader', 'filter'=>false, 'enableSorting'=>false,'content' => function($model, $key, $index, $column) { return $model['is_leader'] ? '是':'否';}),
//            ['class' => 'yii\grid\ActionColumn', 'template' => '{delete}{update}', 'urlCreator'=>function($action, $model, $key, $index){return Url::to(['dcmd-user-group/remove','gid'=>$model['gid'], 'id'=>$model['id']]);}, "visible"=>(Yii::$app->user->getIdentity()->admin == 1 && Yii::$app->user->getIdentity()->sa == 1) ? true : false],
            ['class' => 'yii\grid\ActionColumn', 'template'=>'{update}{delete}', 'urlCreator'=>function($action, $model, $key, $index) {if ("delete" == $action) return Url::to(['dcmd-user-group/remove', 'gid'=>$model['gid'],'id'=>$model['id']]);else return Url::to(['dcmd-user-group/update-user', 'gid'=>$model['gid'], 'id'=>$model['id']]);}, "visible"=>(Yii::$app->user->getIdentity()->admin == 1 && Yii::$app->user->getIdentity()->sa == 1) ? true : false ],
        ],
      ]); ?>
<?= Html::a('添加', ['dcmd-user-group/create', 'gid'=>$gid, 'gname'=>$model->gname], ['class' => 'btn btn-success', (Yii::$app->user->getIdentity()->admin == 1 || Yii::$app->user->getIdentity()->is_leader == 1) ? "" : "style"=>"display:none"]) ?>
</div>
