...
<div class="dcmd-service-pool-audit-index">
 ...
 return Html::a($model->getServicePoolName($model['svr_pool_id']), Url::to(['dcmd-service-pool/view', 'id'=>$model['svr_pool_id']]));},),
        ...
        <?= Html::submitButton('通过', ['class' =>'btn btn-success', (Yii::$app->user->getIdentity()->admin == 1) ? "" : "style"=>"display:none", 'onClick'=>"pass()"])?>&nbsp;&nbsp;
        <?= Html::submitButton('驳回', ['class' =>'btn btn-success', (Yii::$app->user->getIdentity()->admin == 1) ? "" : "style"=>"display:none", 'onClick'=>"reject()"])?>&nbsp;&nbsp;
</div>
</form>
