   ...
   ?>
    <?= GridView::widget([
        ...
        'columns' => [
            ...
            ['class' => 'yii\grid\ActionColumn', 'template' => '{pass} {reject}', 'header' => '操作', 'buttons' => ['pass' => function ($url, $model, $key) { return Html::a('Y', $url, ['title' => '操作通过'] );},'reject' => function ($url, $model, $key) { return Html::a('N', $url, ['title' => '操作驳回'] );}], "visible"=>(Yii::$app->user->getIdentity()->admin == 1) ? true : false],
        ],
    ]); ?>
    <p>
        <?= Html::submitButton('通过', ['class' =>'btn btn-success', (Yii::$app->user->getIdentity()->admin == 1) ? "" : "style"=>"display:none"]) ?> &nbsp;&nbsp;
        <?= Html::submitButton('驳回', ['class' =>'btn btn-success', 'onClick'=>"repeatopr()", (Yii::$app->user->getIdentity()->admin == 1) ? "" : "style"=>"display:none"]) ?>
    </p>
