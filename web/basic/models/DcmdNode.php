...
class DcmdNode extends \yii\db\ActiveRecord
{
    ...
    {
        return [
          //  [['ip', 'ngroup_id', 'host', 'sid', 'did', 'bend_ip', 'utime', 'ctime', 'opr_uid'], 'required'],
            [['ip'], 'required'],
            [['ngroup_id', 'opr_uid'], 'integer'],
            [['utime', 'ctime'], 'safe'],
            [['ip', 'bend_ip', 'public_ip'], 'string', 'max' => 32],
    ...
    public function getAgentVersion($ip)
    {
      $query = DcmdCenter::findOne(['master'=>1]);
    ...
    public function getDcmd_node_group()
    {
        return $this->hasOne(DcmdNodeGroup::className(), ['ngroup_id' => 'ngroup_id']);
    }
