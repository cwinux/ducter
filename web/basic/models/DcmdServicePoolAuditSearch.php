 ...
 */
class DcmdServicePoolAuditSearch extends DcmdServicePoolAudit
{
    /**
     * @inheritdoc
     */
    ...
    public function search($params, $index=true)
    {
        ///应用组用户只可查看所在组的应用
        $svr_pool_con = "";
        if($index==true && Yii::$app->user->getIdentity()->sa != 1)
        {
        ...
        $query = DcmdServicePoolAudit::find()->where($svr_pool_con)->orderBy('ip');
