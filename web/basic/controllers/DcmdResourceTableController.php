 ...
 */
class DcmdResourceTableController extends Controller
{
    ...
    {
        $searchModel = new DcmdResTypeSearch();
    ...
    public function actionView($id)
    {
        ...
        $db=Yii::$app->getDb();
        $r=$db->getSchema()->getTableSchema($model->res_table);
        $fields = \yii\helpers\ArrayHelper::getColumn($r->columns, 'name', false);
        $res_column = DcmdResColumn::find()->andWhere(['res_table'=>$model->res_table])->orderBy('display_order')->asArray()->all();
        $columns = '<table class="table table-striped table-bordered"><tbody>';
        $columns .= '<tr><th>显示名称</th><th>字段名称</th>';
        $columns .= '<tr><td>资源名称</td><td>res_name</td><tr>';
        $columns .= '<tr><td>公司id</td><td>comp_id</td><tr>';
        $columns .= '<tr><td>联系人</td><td>contact</td><tr>';
        foreach($fields as $k=>$v) {
          $exist = false;
          if(!in_array($v,array('res_name','comp_id','contact'))) {
            foreach($res_column as $item) {
              if ($item['colum_name']==$v) {
                $columns .= '<tr><td>'.$item['display_name'].'</td><td>'.$item['colum_name'].'</td><tr>';
                $exist = true;
                break;
              }
            }
            if(!$exist) $columns .= '<tr><td></td><td>'.$v.'</td><tr>';
          }
        }
        $columns .= '</tbody></table>';
    ...
    {
        $model = $this->findModel($id);
        $this->oprlog(3,"delete res_type:".$model->res_type);
        $model->delete();
    ...
    public function actionDeleteAll()
    {
      ...
      foreach($select as $k=>$id) {
        $model = $this->findModel($id);
        $this->oprlog(3, "delete res_type:".$model->res_type);
        $suc_msg .=$model->res_type.':删除成功<br>';
        $model->delete();
      }
    ...
    {
        if (($model = DcmdResType::findOne($id)) !== null) {
            return $model;
        } else {
