...
use app\models\DcmdAppPkgVersion;
...
class DcmdTaskController extends Controller
{
    ...
    public function actionCreate($task_tmpt_id)
    {
        ...
        if(Yii::$app->user->getIdentity()->sa != 1) {
          ...
          $query_svr = DcmdUserGroup::findOne(['uid'=>Yii::$app->user->getId(), 'gid'=>$temp['sa_gid']]);
...
#              $task_pool = DcmdTaskServicePool::find(['svr_pool_id'=>$svr_pool_id])->asArray()->all();
#              if($task_pool) {
#                foreach($task_pool as $item) {
#                  $task_id = $item['task_id'];
#                  $task = DcmdTask::findOne($task_id);
#              
#                  if($task->is_deploy) {
#                    Yii::$app->getSession()->setFlash('error', "对不起, 上线任务:".$task->task_name."中,服务池id:".$svr_pool_id."已存在!");
#                    return $this->redirect(array('dcmd-task/create', 'task_tmpt_id'=>$task_tmpt_id));
#                  }
#                }
#            }
#           }
...
#       }
        ...
        if (Yii::$app->request->post()) { ///提交新建任务
          ...
          $dcmd_task->task_name = Yii::$app->request->post()['task_cmd_prv'];
          ...
          $pkg_version = DcmdAppPkgVersion::findOne(['app_id'=>$post_task['app_id'],'svr_id'=>$post_task['svr_id'],'version'=>$post_task['tag']]);
          if($pkg_version) {
            $dcmd_task->tag_md5 = $pkg_version->md5;
            $dcmd_task->tag_passwd = $pkg_version->passwd;
          }
          $task_cmd = DcmdTaskCmd::findOne(['task_cmd'=>$post_task['task_cmd']]);
          $dcmd_task->is_deploy = $task_cmd->is_deploy;
          ...
          if($dcmd_task->save()) {
            ...
            if (array_key_exists("selection", Yii::$app->request->post())) {
              ...
              foreach($svr_pool as $k=>$svr_pool_id) {
               ...
               $svr_pool_port = DcmdServicePoolPort::find()->andWhere(['svr_pool_id'=>$svr_pool_id])->asArray()->all();
               ...
               $dcmd_task_service_pool->env_md5 = $svr_pool_query['env_md5'];
               $dcmd_task_service_pool->env_passwd = $svr_pool_query['env_passwd'];
               $dcmd_task_service_pool->pool_group = $svr_pool_query['pool_group'];
               $dcmd_task_service_pool->svr_mem = $svr_pool_query['svr_mem'];
               $dcmd_task_service_pool->svr_cpu = $svr_pool_query['svr_cpu'];
               $dcmd_task_service_pool->svr_net = $svr_pool_query['svr_net'];
               $dcmd_task_service_pool->svr_io = $svr_pool_query['svr_io'];
               $dcmd_task_service_pool->image_url = $svr_pool_query['image_url'];
               $dcmd_task_service_pool->image_name = $svr_pool_query['image_name'];
               $dcmd_task_service_pool->image_user = $svr_pool_query['image_user'];
               $dcmd_task_service_pool->image_passwd = $svr_pool_query['image_passwd'];
//               $dcmd_task_service_pool->image_name = "";
               $result = '<?xml version="1.0" encoding="UTF-8" '.'?'.'>';
               $result .= '<ports>';
               if ($svr_pool_port) {
                 foreach($svr_pool_port as $item) {
                   $result .= '<port><name>';
                   $result .= $item['port_name'];
                   $result .= '</name><port>';
                   $result .= $item['port'];
                   $result .= '</port>';
                   $result .= '<mapped_port>';
                   $result .= $item['mapped_port'];
                   $result .= '</mapped_port>';
                   $result .= '</port>';
                }
               }
               $result .= '</ports>';
               $dcmd_task_service_pool->ports = $result;
        ...
        $version_model = DcmdAppPkgVersion::find()->andWhere(['app_id'=>$model['app_id'],'svr_id'=>$model['svr_id']])->asArray()->all();
        $version = array();
    ...
    public function actionCreateByCmd($task_cmd_id) {
            ...
            foreach($svr_pool as $k=>$svr_pool_id) {
              ...
              if($task_pool) {
                foreach($task_pool as $item) {
                  ...
                  if($task->is_deploy) {
                    Yii::$app->getSession()->setFlash('error', "对不起, 上线任务:".$task->task_name."中,服务池id:".$svr_pool_id."已存在!");
                    return $this->redirect(array('dcmd-task/create-by-cmd', 'task_cmd_id'=>$task_cmd_id));
                  }
                }
            }
           }
      ...
      if (Yii::$app->request->post()) { ///提交新建任务
        ...
        $dcmd_task->pause = 0;
        $dcmd_task->tag = $post_task['tag'];
        $dcmd_task->tag_md5 = $post_task['tag_md5'];
        $dcmd_task->tag_passwd = $post_task['tag_passwd'];
        $dcmd_task->is_deploy = $post_task['is_deploy'];
        $dcmd_task->concurrent_rate = $post_task['concurrent_rate'];
        $dcmd_task->timeout = $post_task['timeout'];
        $dcmd_task->auto = $post_task['auto'];
        $dcmd_task->process = $post_task['process'];
    ...
    public function actionGetversion($version,$svr_id) {
      $query = DcmdAppPkgVersion::findOne(['version'=>$version,'svr_id'=>$svr_id]);
      if($query) {
    ...
    public function actionSelectServicePoolNode($task_id)
    {
       ...
       $is_deploy = DcmdTask::findOne($task_id);
       return $this->render('select_service_pool_node', [
         'service_pool' => $service_pool,
         'task_id' => $task_id,
         'task_name' => "task_name",
         'is_deploy' => $is_deploy->is_deploy,
       ]);
    }
    ...
    public function actionGetServiceVersion($svr_id) {
      #return '<tr data-key="15"><td><input type="checkbox" name="selection[]" checked value="15"></td><td>ducter_agent_qcloud1111</td><td>r1.1</td></tr>';
      $query = DcmdAppPkgVersion::find()->andWhere(['svr_id'=>$svr_id])->all();
      $content = '<option value=""></option>';
      foreach($query as $item) {
        $content .= '<option value="'.$item->version.'">'.$item->version.'</option>';
      }
      return $content;
    }
    /**
