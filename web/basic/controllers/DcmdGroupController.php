...
*/
//        $group = DcmdGroup::findOne($id);
        $DcmdUserGroup = DcmdUserGroup::findOne(['uid'=>Yii::$app->user->getId(),'gid'=>$id]);
        if(Yii::$app->user->getIdentity()->sa != 1) {
          if(!$DcmdUserGroup) {
            Yii::$app->getSession()->setFlash('error', "对不起!你没有此权限!");
            $referrer = Yii::$app->request->referrer;
            $url = explode('r=',$referrer)[1];
            Yii::$app->user->setReturnUrl(Yii::$app->request->referrer);
            return $this->goBack();
//            return $this->redirect(array(urldecode($url)));
          } else if($DcmdUserGroup && $DcmdUserGroup->is_leader != 1) {
            Yii::$app->getSession()->setFlash('error', "对不起!你没有此权限!");
            $referrer = Yii::$app->request->referrer;
            $url = explode('r=',$referrer)[1];
            Yii::$app->user->setReturnUrl(Yii::$app->request->referrer);
