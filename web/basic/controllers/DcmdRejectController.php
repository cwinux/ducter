...
use app\models\DcmdAppRejectHistory;
use app\models\DcmdAppRejectHistorySearch;
 ...
 */
class DcmdRejectController extends Controller
{
    ...
    {
        $searchModel = new DcmdAppRejectHistorySearch();
    ...
    public function actionUpdate($id)
    {
        ...
        if(Yii::$app->request->post()) {
          if ($model->load(Yii::$app->request->post())) {
            $model->save();
            $this->oprlog(2,"update image:".$model->id);
    ...
    {
        $model = $this->findModel($id);
        $this->oprlog(3,"delete image:".$model->id);
        $model->delete();
    ...
    public function actionDeleteAll()
    {
      ...
      foreach($select as $k=>$id) {
        $model = $this->findModel($id);
        $this->oprlog(3, "delete image:".$model->name);
    ...
    {
        if (($model = DcmdImage::findOne($id)) !== null) {
            return $model;
        } else {
