...
class DcmdUserController extends Controller
{
    ...
    {
       $gquery = DcmdUser::findOne(['opr_uid'=>Yii::$app->user->getId(),'uid'=>$id]);
     
        if(Yii::$app->user->getIdentity()->admin != 1 && $gquery == NULL) {
          Yii::$app->getSession()->setFlash('success', NULL);
          Yii::$app->getSession()->setFlash('error', "对不起, 你没有权限!");
    ...
    public function actionCreate()
    {
        ...
        $ret_com = DcmdCompany::find()->asArray()->all();
        $company = array();
        foreach($ret_com as $item) {
        ...
        if (Yii::$app->request->post()) {
          ...
          $model->state = 0;
          $model->passwd = md5("123456".Yii::$app->request->post()["DcmdUser"]["username"].strval($model->ctime));
          $model->opr_uid = Yii::$app->user->getId();
          //Yii::$app->getSession()->setFlash('success', '添加用户成功,初始密码:123456');
          if ($model->load(Yii::$app->request->post())) {
            if($model->admin != 1) $model->sa = 0;
            if(Yii::$app->user->getIdentity()->sa != 1) $model->sa = 0;
            if(Yii::$app->user->getIdentity()->admin != 1) $model->admin = 0;
            if( $model->save()) {
             Yii::$app->getSession()->setFlash('success', '添加用户成功,密码为邮箱密码!');
    ...
    {
        $query = DcmdUser::findOne(['opr_uid'=>Yii::$app->user->getId(),'uid'=>$id]);
    ...
    public function actionGetcomp($comp)
    {
      $ret = DcmdDepartment::find()->andWhere(['comp_id'=>$comp])->asArray()->all();
      $dep = array();
      foreach($ret as $items) {
        $dep[$items['depart_id']] = $items['depart_name'];
      }
    ...
    {
       if(Yii::$app->request->post()) {
         $old_passwd = md5(Yii::$app->request->post()['oldpasswd'].Yii::$app->user->getIdentity()->username.strval(Yii::$app->user->getIdentity()->ctime));
         $new_passwd =  md5(Yii::$app->request->post()['newpasswd'].Yii::$app->user->getIdentity()->username.strval(Yii::$app->user->getIdentity()->ctime));
