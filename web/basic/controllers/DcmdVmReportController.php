 ...
 */
class DcmdVmReportController extends Controller
{
    ...
    public function actionIndex()
    {
      ...
      $msg .= '<tr bgcolor="#73BF00"><th>产品名称</th><th>产品别名</th><th>规格</th><th>状态</th><th>数量</th></tr>';
      $vm_report = DcmdPrivate::findBySql('select dcmd_vm_assign.app_name,dcmd_app.app_alias,dcmd_vm_assign.flavor_name,dcmd_vm_assign.state,count(*) from dcmd_vm_assign,dcmd_app where dcmd_vm_assign.app_name = dcmd_app.app_name group by app_name,flavor_name,state')->asArray()->all();
      foreach($vm_report as $k=>$v){
          $msg .= '<tr><td>'.$v["app_name"].'</td><td>'.$v["app_alias"].'</td><td>'.$v["flavor_name"].'</td><td>'.($v["state"] == 0 ? "未使用":"使用中").'</td><td>'.$v["count(*)"].'</td></tr>'; 
      }
      $msg .= '</tbody></table>';
      ...
      $flavor = array();
      $flavor = DcmdPrivate::findBySql('select distinct flavor_name from dcmd_vm_assign')->asArray()->all();
      if($flavor) {
        $flavor = array_column($flavor,"flavor_name");
        sort($flavor);
      }
    ...
    public function actionGetApp($state, $flavor, $app)
    {
      $msg = '<table id="table" class="table table-bordered"><tbody>';
      $msg .= '<tr bgcolor="#73BF00"><th>产品名称</th><th>产品别名</th><th>规格</th><th>状态</th><th>数量</th></tr>';
      $sql = 'select dcmd_vm_assign.app_name,dcmd_app.app_alias,dcmd_vm_assign.flavor_name,dcmd_vm_assign.state,count(*) from dcmd_vm_assign,dcmd_app where dcmd_vm_assign.app_name = dcmd_app.app_name';
      if($app != "ALL") $sql .= ' and dcmd_vm_assign.app_name ='.'"'.$app.'"';
      if($flavor != "ALL") $sql .= ' and dcmd_vm_assign.flavor_name ='.'"'.$flavor.'"';
      if($state != "ALL") $sql .= ' and dcmd_vm_assign.state ='.$state;
      $sql .= ' group by app_name,flavor_name,state';
      $vm_report = DcmdPrivate::findBySql($sql)->asArray()->all();
      foreach($vm_report as $k=>$v){
          $msg .= '<tr><td>'.$v["app_name"].'</td><td>'.$v["app_alias"].'</td><td>'.$v["flavor_name"].'</td><td>'.($v["state"] == 0 ? "未使用":"使用中").'</td><td>'.$v["count(*)"].'</td></tr>';
      }
      $msg .= '</tbody></table>';
