...
//          Yii::$app->getSession()->setFlash('success', NULL);
    ...
    {
        $model = DcmdNodeGroup::findOne($id);
        $query = DcmdUserGroup::findOne(['uid'=>Yii::$app->user->getId(), 'gid'=>$model['gid']]);
        if(!$query && Yii::$app->user->getIdentity()->sa != 1) {
//          Yii::$app->getSession()->setFlash('success', NULL);
          Yii::$app->getSession()->setFlash('error', "对不起, 你没有权限!");
...
//          return $this->redirect(array('dcmd-app/index'));
    ...
    public function actionImportNode($ngroup_id=0)
    {
       ...
       if (Yii::$app->request->post()) {
         ...
         Yii::$app->getSession()->setFlash('success', $err);
       }
    ...
    private function importNodeTxt($fname, $ngroup_id) {
      ...
      if($fd) {
        while(!feof($fd)) {
          $line = fgets($fd);
          $ar = explode(" ",$line);
          if(count($ar) > 1) $msg .= "<font color=red>$line: 格式非法!</font><br>";
          else if($ar[0] != "") {
            $node = new DcmdNode();
            ...
            if($node->save()) $msg .= "$line: 添加成功!<br>";
              ...
              $err_str = "";
              $msg .="<font color=red>$line :添加失败!</font><br>";
            }
          }
        }
      }else {
