...
class DcmdTaskTemplateController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $query = DcmdApp::find()->andWhere($ap_con)->orderBy('app_name')->asArray()->all();
        if($query) {
    ...
    {
        $temp = DcmdTaskTemplate::findOne($id);
        $app = DcmdApp::findOne($temp->app_id);
//        $query = NULL;
//        $query_svr = NULL;
        $query = DcmdUserGroup::findOne(['uid'=>Yii::$app->user->getId(), 'gid'=>$app['sa_gid']]);
        $query_svr = DcmdUserGroup::findOne(['uid'=>Yii::$app->user->getId(), 'gid'=>$app['svr_gid']]);
...
//        }
        ...
        if(Yii::$app->user->getIdentity()->sa != 1) {
          foreach($query as $item) {
            ...
            if(array_key_exists($item['svr_gid'], $sys_ar)) $app[$item['app_id']] = $item['app_name'];
          }
        }else {
          foreach($query as $item) {
