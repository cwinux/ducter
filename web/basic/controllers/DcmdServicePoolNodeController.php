...
class DcmdServicePoolNodeController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $query = DcmdApp::find()->where($app_con)->orderBy('app_alias')->asArray()->all();
        $app = array();
   ...
   public function actionSelectNodeGroup($app_id, $svr_id, $svr_pool_id) 
   {
       ...
       $param = array();
       $param['DcmdAppNodeGroupSearch']['app_id'] = $app_id;
   ...
   public function actionShowNodeList($app_id, $svr_id, $svr_pool_id) {
        ...
        foreach($node_group as $key=>$value) {
          $ngroup_id = DcmdAppNodeGroup::findOne($value);
          $value = $ngroup_id['ngroup_id'];
...
#         $server_pool = new DcmdServicePoolNode();
#         $server_pool->svr_pool_id = $svr_pool_id;
#         $server_pool->svr_id = $svr_id;
#         $server_pool->app_id = $app_id;
#         $server_pool->nid = $item['nid'];
#         $server_pool->ip = $item['ip'];
#         $server_pool->utime = $tm;
#         $server_pool->ctime = $tm;
#         $server_pool->opr_uid = Yii::$app->user->getId();
#         $server_pool->save();
    ...
    public function actionDeleteSelect()
    {
      ...
      foreach($select as $k=>$id) {
        $model = DcmdServicePoolNode::findOne($id);
        $svr_pool_id = $model->svr_pool_id;
