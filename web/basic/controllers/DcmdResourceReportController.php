 ...
 */
class DcmdResourceReportController extends Controller
{
    ...
    public function actionIndex()
    {
      ...
      $msg = '<table id="table" class="table table-bordered"><tbody>';
      $msg .= '<tr bgcolor="#73BF00"><th>区域</th><th>类别</th><th>运营商</th><th>机房</th><th>集群名称</th><th>管理地址</th><th>虚拟机实例地址</th><th>计算节点</th><th>超售比</th><th>可容纳实例</th><th>已创建实例</th><th>可创建实例</th><th>实例过多</th></tr>';
      $total_app = 0;
      $total_node = 0;
      $total_all_vm = 0;
      $total_create_vm = 0;
      ...
      if($query){
        foreach($query as $k=>$v) {
          if($v["self_service"] == 1) {
            $dcInfo = DcmdDcInfo::findOne(['dc'=>$v["dc"]]);
            $count = DcmdServicePoolNode::findBySql('select count(*) from dcmd_service_pool_node where app_id=:app_id and svr_id=(select svr_id from dcmd_service where svr_name="compute" and app_id=:app_id)',array(":app_id"=>$v["app_id"]))->asArray()->all();
            $vm_count = DcmdPrivate::findBySql('select count(*) from dcmd_vm_assign where host_ip in (select ip from dcmd_service_pool_node where app_id=:app_id and svr_id=(select svr_id from dcmd_service where svr_name="compute" and app_id=:app_id))',array(":app_id"=>$v["app_id"]))->asArray()->all();
//            print_r($count);
            $left = (($count[0]["count(*)"]*$v["oversale_rate"])>=($vm_count[0]["count(*)"])) ? (($count[0]["count(*)"]*$v["oversale_rate"])-($vm_count[0]["count(*)"])):"NULL";
            $total_app += 1;
            $total_node += $count[0]["count(*)"];
            $total_all_vm += ($count[0]["count(*)"]*$v["oversale_rate"]);
            $total_create_vm += $vm_count[0]["count(*)"];
            $total_valable_vm += is_int($left) ? $left:0;
            $msg .= '<tr><td bgcolor="6FB7B7">'.(is_object($dcInfo) ? $dcInfo->area : "").'</td><td bgcolor="6FB7B7">'.$v["type"].'</td><td bgcolor="6FB7B7">'.(is_object($dcInfo) ? $dcInfo->country : "").'</td><td bgcolor="6FB7B7">'.$v["dc"].'</td><td bgcolor="#EAC100">'.$v["app_name"].'</td><td bgcolor="#EAC100">'.$v["vip"].'</td><td bgcolor="#ADADAD">'.$v["net"].'</td><td bgcolor="#ADADAD">'.$count[0]["count(*)"].'</td><td bgcolor="#ADADAD">1:'.$v["oversale_rate"].'</td><td bgcolor="#ADADAD">'.($count[0]["count(*)"]*$v["oversale_rate"]).'</td><td bgcolor="#ADADAD">'.$vm_count[0]["count(*)"].'</td><td bgcolor="#ADADAD">'.$left.'</td><td bgcolor="#ADADAD">NULL</td></tr>'; 
          }
        }
      }
      $msg .= '<tr><th colspan="13">汇总</th></tr>';
      $msg .= '<tr bgcolor="#73BF00"><td>集群数</td><td>节点数</td><td>可容纳实例</td><td>已创建实例</td><td>可创建实例</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>';
      $msg .= '<tr><td>'.$total_app.'</td><td>'.$total_node.'</td><td>'.$total_all_vm.'</td><td>'.$total_create_vm.'</td><td>'.$total_valable_vm.'</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>';
      $msg .= '</tbody></table>';

      $dc_array = DcmdDcInfo::find()->asArray()->all();
      $dc_info = array();
      $operate = array();
      if($dc_array) {
        $dc_info = array_column($dc_array,"area");
        $dc_info = array_unique($dc_info);
        $operate = array_column($dc_array, "country");
        $operate = array_unique($operate);
        $operate = array_filter($operate);
      }
      $type = array();
      $app = array();
      if($query) {
        $type = array_column($query,"type");
        $type = array_unique($type); 
        $type = array_filter($type);
        $app = array_column($query,"app_name");
        sort($app);
      }
    ...
    public function actionGetApp($area, $type, $operate, $app)
    {
      if($app == "ALL") $query = DcmdApp::find()->asArray()->all();
      else $query = DcmdApp::find()->andWhere(["app_name" => $app])->asArray()->all();
      $msg = '<table id="table" class="table table-bordered"><tbody>';
      $msg .= '<tr bgcolor="#73BF00"><th>区域</th><th>类别</th><th>运营商</th><th>机房</th><th>集群名称</th><th>管理地址</th><th>虚拟机实例地址</th><th>计算节点</th><th>超售比</th><th>可容纳实例</th><th>已创建实例</th><th>可创建实例</th><th>实例过多</th></tr>';
      $total_app = 0;
      $total_node = 0;
      ...
      if($query){
        foreach($query as $k=>$v) {
          if($v["self_service"] == 1) {
            $dcInfo = DcmdDcInfo::findOne(['dc'=>$v["dc"]]);
            if($area !="ALL" && $dcInfo->area != $area)  continue;
            if($operate !="ALL" && $dcInfo->country != $operate) continue;
...
//            print_r($count);
    ...
    public function actionUpdateFault($fault_id,$fault_title,$fault_area,$custom_type,$custom_name,$fault_date,$case_num,$fault_level,$find_way,$fault_show,$sustained_time,$start_time,$discover_time,$handle_time,$recover_time,$reason,$reason_type,$response_model,$response_person,$handle_process,$improve)
    {
      $model = $this->reportModel($fault_id);
    ...
    {
      $model = DcmdPrivate::find()->andWhere(["host_ip"=>$host_ip])->asArray()->all();
      $model1 = DcmdPrivate::find()->andWhere(["vm_ip"=>$host_ip])->asArray()->all();
      $query = $model+$model1;
      $custom = array();
      $customs = "";
