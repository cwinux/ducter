 ...
 */
class DcmdNetResourceController extends Controller
{
...
//      return strlen($blp);
    ...
    public function actionSegmentList($net)
    {
      $arr = DcmdAppSegment::find()->asArray()->all();
      foreach($arr as $key=>$value) {
        if(!($this->findSegment($net,$value))) unset($arr[$key]);
      }
      return json_encode(array_values($arr));
    }

    public function actionNetList()
    {
      $arr = DcmdIdcNetwork::find()->asArray()->all();
      $data = array();
      foreach($arr as $k=>$v) {
        $value = explode(".", $v["segment"]);
