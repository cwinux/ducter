...
use app\models\DcmdCbsNodeStatusSearch;
 ...
 */
class DcmdCbaseAppNodeController extends Controller
{
    ...
    {
        $searchModel = new DcmdCbsNodeStatusSearch();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
   ...
   public function actionAddNode() {
     ...
     if (array_key_exists("selection", Yii::$app->request->post())){
       ...
       foreach($query as $item) {
         ...
         $dcmd_cbase_app_node->status = 'Up';
         $dcmd_cbase_app_node->ctime = $tm;
         $dcmd_cbase_app_node->utime = $tm;
    ...
    public function actionDeleteNode($id, $svr_pool_id=NULL)
    {
      $model = $this->findModel($id);
      $this->oprlog(3,"delete cbsase pool node:".$model->ip);
      $model->delete();
      Yii::$app->getSession()->setFlash('success', '删除成功!');
      return $this->redirect(array('dcmd-cbase-app-node/index'));
    }
