 ...
 */
class DcmdAlarmHistoryController extends Controller
{
    ...
    {
        $searchModel = new DcmdAlarmHistorySearch();
   ...
   public function actionReject() {
    ...
    public function actionDeleteAll()
    {
      ...
      foreach($select as $k=>$id) {
        $model = $this->findModel($id);
        $this->oprlog(3, "delete alarm history:".$model->alarm_id);
        $suc_msg .=$model->alarm_id.':删除成功<br>';
        $model->delete();
      }
    ...
    {
        if (($model = DcmdAlarmHistory::findOne($id)) !== null) {
            return $model;
        } else {
