...
use app\models\DcmdIdc;
use app\models\DcmdIdcSearch;
 ...
 */
class DcmdIdcController extends Controller
{
    ...
    {
        $searchModel = new DcmdDcInfoSearch();
    ...
    public function actionCreateNetwork($dc)
    {
        $model = new DcmdIdcNetwork();
        $segment = DcmdDcInfo::findOne($dc);
//        $dc = $segment->dc;
        if($model->load(Yii::$app->request->post())) {
         //   $model->dc = $dc;
            $model->save();
            $this->oprlog(1, "add idc's network:".$model->idc);
            Yii::$app->getSession()->setFlash('success', '添加成功!');
            return $this->redirect(['view', 'id' => $dc,]);
        }
...
//      return strlen($blp);
    ...
    public function actionUpdate($id)
    {
        ...
        if(Yii::$app->request->post()) {
          if ($model->load(Yii::$app->request->post())) {
            $model->save();
            $this->oprlog(2,"update network:".$model->dc_id);
            Yii::$app->getSession()->setFlash('success', "修改成功");
            return $this->redirect(['view', 'id' => $model->dc_id]);
          } 
         $err_str = "";
    ...
    {
        $model = $this->idcModel($id);
        $this->oprlog(3,"delete idc info:".$model->dc);
        $model->delete();
    ...
    public function actionDeleteNet()
    {
    ...
    //    return $this->redirect(['index']);
      ...
      $err_msg = "";
      foreach($select as $k=>$id) {
        $model = DcmdIdcNetwork::findOne($id);
        $this->oprlog(3, "delete idc network:".$model->id);
        $suc_msg .=$model->id.':删除成功<br>';
        $model->delete();
      }
      if($suc_msg != "") Yii::$app->getSession()->setFlash('success', $suc_msg);
      if($err_msg != "") Yii::$app->getSession()->setFlash('error', $err_msg);
      return $this->redirect(['view', 'id' => $model->idc]);
    }

    public function actionNetUpdate($id)
    {
        $model = $this->netModel($id);
        $idc = $this->getIdc();
        if(Yii::$app->request->post()) {
          if ($model->load(Yii::$app->request->post())) {
            $model->save();
        ...
        return $this->render('net-update', [
             'model' => $model,
             'idc' => $idc,
        ]);
    }

    public function actionNetDelete($id)
    {
        $model = $this->netModel($id);
        $this->oprlog(3,"delete idc info:".$model->id);
        $model->delete();
        Yii::$app->getSession()->setFlash('success', '删除成功!');
    ...
    protected function netModel($id)
    {
    ...
    {
      $query = DcmdDcInfo::find()->asArray()->all();
      $idc = array();
      foreach($query as $v)
        $idc[$v['dc_id']] = $v['dc'];
      return $idc;
    ...
    {
        if (($model = DcmdDcInfo::findOne($id)) !== null) {
            return $model;
        } else {
            throw new NotFoundHttpException('The requested page does not exist.');
        }
